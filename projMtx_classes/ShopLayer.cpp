...
bool ShopLayer::init() {
    ...
    /**********TEST************/
//    LoadingBox* l = LoadingBox::create();
//    l->show();
    /**********TEST************/
    
    CCSprite* sp = CCSprite::create("c_caoc2.png");
    sp->setAnchorPoint(ccp(0.5, 0));
    sp->setPosition(ccp(320, 400));
    sp->setColor(ccc3(128,128,105));
    addChild(sp, 1000);
    
    sp->runAction(CCMoveBy::create(15, ccp(0, -200)));
...
void ShopLayer::moveOutTouchUICompent() {
//    this->setVisible(false);
    CCNode* pNode = (CCNode*)this->getChildByTag(CONTAINER);
    pNode->runAction(CCSequence::create(CCMoveTo::create(OPENNING_ANI_DURING, ccp(0, -LAYER_OFFSET)), NULL));
}

void ShopLayer::restoreTouchUICompent() {
